...
s_exp = 7 #exponent for the number of samples
...
output_type = 'bin'
dust_grain = 'cratering'

def run_test(trial_number):
    """runs the Monte Carlo simulation once"""
    start_time = time.time()
    radius_generator = slice_sampling(cratering, m_cra)
    ...
    elif dust_grain == 'cratering':
        depths = ((e,(d*10**(-8)),(r/100),0 if (d*10**(-8)) < (r/100) else 1)
                        for r,e in islice(izip(radius_generator,energy_generator),number_of_samples)
                        for d in (depth_functions[e*keV],))
    if output_type == 'csv':
...
if __name__ == "__main__":
    config.accuracy = 0.1
    number_of_trials = 5 #runs this number of trials
